...
def _load_config_module_with_safe_defaults() -> types.ModuleType:
    def default_requests_get(
        ...
        if url.endswith("/instances"):
            return FakeResponse(
                payload={"items": [{"connectionName": "project-1:region:live"}]}
            )
...
def test_discover_destination_instance_name_from_api() -> None:
    ...
    assert instance_name == "project-1:region:live"
...
def test_discover_destination_instance_name_falls_back_to_gcloud() -> None:
    ...
    with (
        ...
        patch(
            "config.subprocess.run",
            return_value=SimpleNamespace(stdout="project-1:region:live\n"),
        ),
    ):
        instance_name = config_module.discover_destination_instance_name("project-1")
...
def test_discover_destination_instance_name_prefers_non_clone_single_choice() -> None:
    config_module = _load_config_module_with_safe_defaults()
    api_response = FakeResponse(
        payload={
            "items": [
                {"connectionName": "project-1:region:pitr-clone"},
                {"connectionName": "project-1:region:live"},
            ]
        }
    )
...
def test_discover_destination_instance_name_exits_when_ambiguous() -> None:
    ...
    api_response = FakeResponse(
        payload={
            "items": [
                {"connectionName": "project-1:region:live-a"},
                {"connectionName": "project-1:region:live-b"},
            ]
        }
    )
    ...
    with (
        patch("config._discover_access_token", return_value="token"),
        patch("config.requests.get", return_value=api_response),
        pytest.raises(SystemExit, match="Multiple Cloud SQL instances found"),
    ):
...
def test_discover_database_name_returns_sorted_first_without_preferred_name() -> None:
    config_module = _load_config_module_with_safe_defaults()
    api_response = FakeResponse(
...
def test_settings_class_is_initialized_with_discovered_values() -> None:
    ...
    assert settings.DEST_INSTANCE_NAME == "project-1:region:live"
    assert settings.RESTORE_SOURCE_INSTANCE_NAME == settings.DEST_INSTANCE_NAME
    assert settings.DEST_DB_NAME == "blaise"
    assert settings.DEST_DB_DRIVER == "pymysql"
    assert settings.DEST_DB_URL == "mysql+pymysql://"
    assert settings.DEST_DB_USERNAME == "blaise"
    assert settings.DEST_DB_PASSWORD == "password"
