...
class TestOrmFunctionality:
    ...
    def test_copy_table_data_deletes_any_existing_data_from_the_destination_table(
        self,
        source_table,
        destination_table,
        service_under_test,
        ...
        print("2nd test begins:")
        print(fake_destination_database_table_service.get_records())
        # arrange
        fake_source_database_table_service.add_record(
            source_table(FormID=1, Serial_Number=900001)
        )
        ...
        fake_destination_database_table_service.add_record(
            destination_table(FormID=4, Serial_Number=900004)
        )
        fake_destination_database_table_service.add_record(
            destination_table(FormID=5, Serial_Number=900005)
        )
        fake_destination_database_table_service.add_record(
            destination_table(FormID=6, Serial_Number=900006)
        )
